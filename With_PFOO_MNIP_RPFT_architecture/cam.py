...
def patch_detection(frame, detections, sizes, conf_threshold=0.5):
    """
    Runs YOLO on each PFOO patch and updates `detections` and `sizes`
    so that only YOLO-filtered high-confidence objects remain.
    """
    ...
    for (x, y, w, h) in detections:
        ...
        # Crop patch from frame
        patch = frame[y:y+h, x:x+w]
        if patch.size == 0:
            continue

        # YOLO inference
        results = yolo_model.predict(patch, verbose=False)
        ...
        for r in results:
            for box in r.boxes:
                ...
                # Convert patch coords → full-frame coords
                abs_x = int(x + x1)
                abs_y = int(y + y1)

                filtered_detections.append([abs_x, abs_y, bw, bh])
                filtered_sizes.append([bw, bh])
